...
public class Cepr01040227Business extends Cepr01040000Ilustration134Business
{
    protected final Log logger = LogFactory.getLog( getClass() );

    public Cepr01040227Business( EproposalManager eproposalManager, Cepr00000000EditorUtil editorUtil, Object command )
    {
        super( eproposalManager, editorUtil, command );
        setSekaligusRightPartCdArr( new Integer[]{ } );
        setBerkalaRightPartCdArr( new Integer[]{ Cepr01040227Mapper.X1 } );
        ...
        setInvestmentTitleArr( new String[] {
                ...
                productLabel + " Secure Dollar Income Fund Syariah",
                productLabel + " Dynamic Dollar Fund Syariah"
            } );
    	
             		
        setInvestmentTitleAndDescrArr( new String[][] {
            { "", "", "" },
            { "", "- " + "Excellink Fix Income Fund Syariah", "Penempatan minimum 80% (delapan puluh perseratus) dana investasi pada  instrumen-instrumen investasi berpendapatan tetap dan/atau maksimum 20% (dua puluh perseratus) instrumen-instrumen investasi pasar uang." },
            ...
            { "", "- Excellink Dynamic Dollar Fund Syariah", "Penempatan minimum 50% pada Fixed Income dan maksimum 50% pada Equity, dimana semua instrumen-instrumen investasi tersebut dalam mata uang US Dollar" },
        } );
...
//        double ldec_premi_invest;
        ...
        double[] ldec_tarik = { CommonConst.DUMMY_ZERO, 0, 0, 0, 0, 0, 0 };
        if( cepr01030102Form.getRightPartOfBusinessCd() == Cepr01040208Mapper.X1){    		
      		 ldec_tarik = new double[] { CommonConst.DUMMY_ZERO, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.05};
          }
        double ldec_wdraw;
        double[] ldec_premi_bulan = new double[12 + 1];
...
////Biaya Akuisisi & asumsi penarikan (1 tarik, 1-nya tdk tarik)
        ...
        int j;
        Integer flag_syarat_bonus_premi = flag_syarat_bonus_premi(lstr);
...
//        			ldec_premi_invest = 0;
        			...
        			//if(li_bulan == 1) ldec_topup = ldec_topup_temp* ( 1 - ldec_bak_tu);
        			if(li_bulan == 1) {
        			...
        			if(istr_prop.pct_premi != 100 || (istr_prop.pct_premi == 100 && cekAdaTopUp) ){
        				...
        				npbatal[ k ] = FormatNumber.round((  ldec_hasil_ppokok[k] - (  ldec_hasil_ppokok[k] * ldec_sc) +  ldec_hasil_pptu[k])/li_bagi ,0);
        				
        			}
        			else if(istr_prop.pct_premi == 100 && !cekAdaTopUp ){
        			...
        			ldec_hasil_total[k] = (ldec_hasil_ppokok2[k] + ldec_ttl_byi_prmtu_checking[k]); 
        			 ...
        			 npbatal[ k ] = FormatNumber.round(  ( ldec_hasil_total[k] -   (ldec_hasil_total[k] * ldec_sc)   )/ li_bagi ,0);
        			}
...
//                    npbatal[ k ] = FormatNumber.round((  ldec_hasil_ppokok[k] - (  ldec_hasil_ppokok[k] * ldec_sc) +  ldec_hasil_pptu[k])/li_bagi ,0);
                    //npbatal[ k ] = FormatNumber.round( ( (ldec_hasil_ppokok[k] * (1 - ldec_sc) ) + ldec_hasil_pptu[k] )/ li_bagi, 0 );
                }
...
//                    npbatal[ k ] = FormatNumber.round((  ldec_hasil_ppokok[k] - (  ldec_hasil_ppokok[k] * ldec_sc) +  ldec_hasil_pptu[k])/li_bagi ,0);
    ...
    public List<Map<String, Object>> getCommonHeaderRincianRowList() throws IOException
    {
        ...
        if( cepr01030102Form.getLeftPartOfBusinessCd() == 153 || cepr01030102Form.getLeftPartOfBusinessCd() == 160 || cepr01030102Form.getLeftPartOfBusinessCd() == 200 ||
            	cepr01030102Form.getLeftPartOfBusinessCd() == 202 || cepr01030102Form.getLeftPartOfBusinessCd() == 218 || cepr01030102Form.getLeftPartOfBusinessCd() == 216){ //SYARIAH
