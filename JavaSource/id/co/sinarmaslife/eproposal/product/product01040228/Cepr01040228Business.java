...
public class Cepr01040228Business extends Cepr01040000Ilustration134Business
{
    protected final Log logger = LogFactory.getLog( getClass() );

    public Cepr01040228Business( EproposalManager eproposalManager, Cepr00000000EditorUtil editorUtil, Object command )
    {
        super( eproposalManager, editorUtil, command );
        setSekaligusRightPartCdArr( new Integer[]{ } );
        setBerkalaRightPartCdArr( new Integer[]{ Cepr01040228Mapper.X1, Cepr01040228Mapper.X2, Cepr01040228Mapper.X3, Cepr01040228Mapper.X4} );
        ...
        if( cepr01030102Form.getRightPartOfBusinessCd() == Cepr01040228Mapper.X3 ){
	        ...
	        setInvestmentTitleAndDescrArr( new String[][] {
	            { "", "", "" },
	            { "", "- " + "Excellink Fixed Income Fund", "Penempatan minimum 80% (delapan puluh perseratus) dana investasi pada  instrumen-instrumen investasi berpendapatan tetap dan/atau maksimum 20% (dua puluh perseratus) instrumen-instrumen investasi pasar uang." },
	            { "", "- " + "Excellink Dynamic Fund", "Penempatan minimum 50% pada Fixed Income dan maksimum 50% pada Equity. Investasi berpendapatan tetap, ekuitas serta instrumen pasar uang" },
	            { "", "- " + "Excellink Aggresive Fund", "Penempatan minimum 80% (delapan puluh perseratus) dana investasi pada instrumen-instrumen investasi pasar modal (ekuitas) dan/atau maksimum penempatan 20% (dua puluh perseratus) dana investasi pada instrumen-instrumen investasi pasar uang. " },
	            { "", "- " + "Excellink Cash Fund", "Penempatan dana investasi 100% pada instrumen investasi pasar uang" },
	            { "", "- " + "Excellink Equity Bakti Peduli", " Penempatan minimum 80% (delapan puluh perseratus) dana investasi instrumen-instrumen investasi pasar modal (ekuitas) dan maksimum 20% (dua puluh perseratus) instrumen-instrumen investasi pasar uang" },
	            ...
	            { "", "- Excellink Dynamic Dollar Fund", "Penempatan minimum 50% pada Fixed Income dan maksimum 50% pada Equity" },
	        } );
        }
...
////monthly fix cost
        ...
        }else if(cepr01030102Form.getRightPartOfBusinessCd() == Cepr01040228Mapper.X4 || cepr01030102Form.getRightPartOfBusinessCd() <= Cepr01040228Mapper.X2){
	       	 if( "01".equals( istr_prop.kurs_id ) )
	         {
	    		 a = 1;
	             b = 8;    
...
//        	surrender charge
    ...
    public double flag_syarat_bonus_premi(int i, Cepr01030102Form cepr01030102Form , int k , double ldec_premi_ppokok_setahun){
    	double result = 0;
    	//iga 04082020 update investasi fund
    	if( cepr01030102Form.getRightPartOfBusinessCd() == Cepr01040228Mapper.X4 ){
        ...
        for( i = a; i <= b; i++ )
        {
            ...
            if( cepr01030102Form.getRightPartOfBusinessCd() == Cepr01040228Mapper.X3 ){
           	 ...
           	 if( ("01".equals( istr_prop.kurs_id ) && i<9) ||  ("02".equals( istr_prop.kurs_id ) && i<11))
           	 {
            	 ...
            	 if( ("01".equals( istr_prop.kurs_id ) && i<6) ||  ("02".equals( istr_prop.kurs_id ) && i<8))
                 {
     	            totalInvestmentAllocation = totalInvestmentAllocation + istr_prop.fund[ j ];
     	            totalAsumptionLow = totalAsumptionLow + allocationYieldLow;
     	            totalAsumptionMid = totalAsumptionMid + allocationYieldMid;
