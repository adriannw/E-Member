 ...
 * Function Id         	: 
 * Program Name   		: Cepr01040213Business
 * Description         	:
...
public class Cepr01040213Business extends Cepr01040000Ilustration116Business {
	protected final Log logger = LogFactory.getLog(getClass());

	public Cepr01040213Business(EproposalManager eproposalManager, Cepr00000000EditorUtil editorUtil, Object command) {
		super(eproposalManager, editorUtil, command);
		// setSekaligusRightPartCdArr( new Integer[]{ Cepr01040213Mapper.X5,
		// Cepr01040213Mapper.X6 } );
		setBerkalaRightPartCdArr(new Integer[] { Cepr01040213Mapper.X5, Cepr01040213Mapper.X6, Cepr01040213Mapper.X7,
				Cepr01040213Mapper.X8, Cepr01040213Mapper.X9, Cepr01040213Mapper.X10 });
	...
	public List<Map<String, Object>> getPageList(int currentIdx) throws IOException {
		...
		// int flag_top_draw = 0;
		// for( TopupDrawVO x : cepr01030104Form.getTopupDrawVOList() ){
		// if( x.getDrawAmount().compareTo( BigDecimal.ZERO ) > 0 ||
		// x.getTopupAmount().compareTo( BigDecimal.ZERO ) > 0 ){
		// flag_top_draw += 1;
		// }
		// }
		// if( flag_top_draw > 0 ){
	...
	public String[][] getRiderPremiumList(Cepr01030102Form cepr01030102Form) throws IOException {
		...
		String[][] riderTambahan = new String[7][7];
		...
		// if Pos('03, 04, 09, 10', string(istr_prop.bisnis_no, '00#')) > 0 Then
		// //cerdas siswa B 5/10
		// istr_prop.rider_baru[4] = 1 //813 ci
		// if Pos('03, 09', string(istr_prop.bisnis_no, '00#')) > 0 Then
		// istr_prop.rider_baru[7] = 4 //816 waiver ci
		// End If
		// if Pos('04, 10', string(istr_prop.bisnis_no, '00#')) > 0 Then
		...
		if (istr_prop.rider_baru[3] > 0)
			istr_prop.rider_baru[3] = 2; // 812 TPD
		if (cepr01030102Form.getRightPartOfBusinessCd() == 3 || cepr01030102Form.getRightPartOfBusinessCd() == 4
				|| cepr01030102Form.getRightPartOfBusinessCd() == 9
				|| cepr01030102Form.getRightPartOfBusinessCd() == 10) {
			if (istr_prop.rider_baru[4] > 0)
				istr_prop.rider_baru[4] = 1; // 813 ci
			if (cepr01030102Form.getRightPartOfBusinessCd() == 3 || cepr01030102Form.getRightPartOfBusinessCd() == 9) {
	...
	public String of_nama(int ai_no, Cepr01030102Form cepr01030102Form, Cepr01030103Form cepr01030103Form) {
		...
		case 5:
			...
			break;
		case 6:
			ls_nama = "Payor 25 TPD/Death" + syariah;
			break;
		case 7:
			ls_nama = "Waiver CI" + syariah;
			break;
		case 8:
			ls_nama = "Payor 25 CI/Death" + syariah;
			break;
		case 9:
	...
	public Map<String, Object> getIllustration213Result(int lamaBayar) {
		...
		if (istr_prop.kurs_id.equals("02")) {
			ldec_mfc = 1.5;
			if ("120, 127, 128, 129".indexOf(LazyConverter.toString(istr_prop.bisnis_id)) > -1) {
		...
		for (int i = 1; i <= istr_prop.ins_per; i++) {
			...
			/*
			 * if( cepr01030102Form.getLeftPartOfBusinessCd() == 141 ) { if( i == 1 ) {
			 * //ass basic dinolin, ass rider tidk 23/08/07 ldec_temp =
			 * of_get_coi_120_basic( i ); ldec_coi = ldec_coi - ldec_temp; //ldec_temp =
			 * ldec_temp / 2 //distahunkan /24 ldec_mfc = 0; //ass rider pa, hcp, hcp fam
			 * ada (3/9/07) for( int k = 1; k <= ArrUtil.upperBound( istr_prop.rider_baru );
			 * k++ ) { if( !( ( k == 1 ) || ( k == 2 ) || ( k == 11 ) ) ) { if(
			 * istr_prop.rider_baru[ k ] > 0 ) { ldec_coi -= of_get_coi_120_rider( k, 1 );
			 * ldec_temp += of_get_coi_120_rider( k, 1 ); } } else { //pa, hcp, hcp fam tdk
			 * ditunda } } ldec_temp = ldec_temp / 24; //distahunkan /24 } else if( i >= 3
			 * && i <= 4 ) { ldec_coi24 = ldec_temp; } else { ldec_coi24 = 0; }
			 * 
			 * if( i >= 2 ) { ldec_mfc = 45000; //30000 16/07/07 if(
			 * Hardcode.CUR_USD_CD.equals( istr_prop.kurs_id ) ) { ldec_mfc = 5; }
			 * //tambahin per 10/8/07 if( istr_prop.cb_id == 0 ) { ldec_mfc = 30000; if(
			 * Hardcode.CUR_USD_CD.equals( istr_prop.kurs_id ) ) { ldec_mfc = 4; } } } }
			 */

			ldec_cost = (ldec_coi + ldec_mfc);
			// logger.info("----***ldec_coi24" + ldec_coi24);
			...
			for (int k = 1; k <= 3; k++) {
				for (int li_bulan = 1; li_bulan <= 12; li_bulan++) {
					...
					// logger.info("***------ldec_bunga_avg"+ ldec_bunga_avg[ k ]);
					// logger.info("***------ldec_hasil_investxxx"+i+" "+ ( ldec_premi_invest +
					// ldec_hasil_invest[1][k] - ldec_cost) * FormatNumber.round(Math.pow( ( 1 +
					// ldec_bunga_avg[ k ] ), ( ( double ) 1 / 12 )), 15));

					// ldec_hasil_invest[1][k] = FormatNumber.round(( ldec_premi_invest +
					// ldec_hasil_invest[1][k] - ldec_cost) * FormatNumber.round(Math.pow( ( 1 +
					// ldec_bunga_avg[ k ] ), ( ( double ) 1 / 12 )), 15),2);
				...
				if (lb_hal_tarik) {
					if (istr_prop.bisnis_id == 127 || istr_prop.bisnis_id == 141) {
						...
						if (istr_prop.bisnis_no == 5 || istr_prop.bisnis_no == 6) {
				...
				if (ldec_hasil_invest[1][k] <= 0) {
					...
					if (istr_prop.kurs_id.equals("02"))
						ldec_maks_premi_tahunan = 10000.0;
					if (i <= 20) {
						lb_minus[k] = true;
