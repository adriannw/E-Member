 ...
 * Function Id         	: 
 * Program Name   		: Cepr01040109DownloadController
 * Description         	: Platinum Save (155)
 * Environment      	: Java  1.5.0_06
 * Author               : samuel
 * Version              : 1.0
 * Creation Date    	: Jan 7, 2008 2:35:44 PM
 *
...
import id.co.sinarmaslife.eproposal.product.product01040108.Cepr01040108Business;
...
public class Cepr01040109DownloadController extends StandardParent implements Controller
{
    ...
    {
        logger.info( "*-*-*-* Cepr01040109DownloadController.handleRequest" );
        ...
        Cepr01040109Business cepr01040109Business = new Cepr01040109Business( eproposalManager, editorUtil, command );
        ...
        else
        {
            ...
            jasperViewer.setUrl( "/WEB-INF/classes/" + "id/co/sinarmaslife/eproposal/source/report/jasper/xepr_01040109_product.jasper" );
...
//            Date nowDate = eproposalManager.selectNowDate();
            ...
            {
                ldec_premi = premium;
                premium = FormatNumber.truncate( ldec_premi * ( ( 1 / Math.pow( ( 1 + (investRate/100)), ldec_mgi ) ) ), 2 );
                if( Hardcode.TAX_AFTER_CD.equals( cepr01030102Form.getTaxBeforeAfterCd() ) )
                {
                    if( mgi < 12 )
                    {
                        premium = FormatNumber.truncate( ldec_premi / ( 0.2 + 0.8 * ( Math.pow( ( 1 + ( investRate/100 ) ), ldec_mgi ) ) ), 2);
                    }
                }
            ...
            ciRider = cepr01040109Business.of_get_coi_ci(kursId, cepr01030101Form.getInsuredAge(), premiTotalSekaligus, premiTotalTahunan, premiTotalSemesteran, premiTotalTriwulanan, premiTotalBulanan);
            termRider = cepr01040109Business.of_get_coi_term(kursId, cepr01030101Form.getInsuredAge(), premiTotalSekaligus, premiTotalTahunan, premiTotalSemesteran, premiTotalTriwulanan, premiTotalBulanan);
            if(jenis != 0){
            	hcpFamilyRider = cepr01040109Business.of_get_coi_hcp_family(jenis, kursId, cepr01030101Form.getInsuredAge(), premiTotalSekaligus, premiTotalTahunan, premiTotalSemesteran, premiTotalTriwulanan, premiTotalBulanan);
            ...
            for( double li_loop = 1; li_loop <= li_years; li_loop++ )
            {
                ...
                if( mgi != 36 )
                {
                    ...
                    {
                        ldec_saldo += FormatNumber.truncate( li_hari / 365 * ldec_saldo * ( investRate / 100 ) * 0.8, 2);
                    }
                    else
                    {
                        ldec_saldo += FormatNumber.truncate( li_hari / 365 * ldec_saldo * ( investRate / 100 ), 2 );
                    }
                }
                else
                {
                    if( Hardcode.TAX_AFTER_CD.equals( cepr01030102Form.getTaxBeforeAfterCd() ) && ( li_loop / li_bagi < 3 ) )
                    {
                        ldec_saldo = FormatNumber.truncate( ( premium * Math.pow( 1 + investRate / 100, li_loop / li_bagi ) - premium ) * 0.8 + premium, 2);
                    }
                    else
                    {
                        ldec_saldo = FormatNumber.truncate( premium * Math.pow( 1 + investRate / 100, li_loop / li_bagi ), 2 );
                    }
                }
            ...
            params.put( "title", "PLATINUM SAVE" );
...
//            params.put( "placeAndDate", "Jakarta, " + FormatDate.toIndonesian( eproposalManager.selectNowDate() ) );
           ...
           try{
	        	   ...
	        	   //if(cekDataApprovedOrNot.get(0).getPosisi()<3){
			            JasperUtils.exportReportToPdf(
			            		"id/co/sinarmaslife/eproposal/source/report/jasper/xepr_01040109_product.jasper",
			            		dirName,
			                fileName,
							params,
							temp,
    ...
    {
        logger.info( "*-*-*-* Cepr01040109DownloadController.getCommonHeaderRowList" );
