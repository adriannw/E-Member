...
public class Cepr01040218Business extends Cepr01040000Ilustration116Business {
	protected final Log logger = LogFactory.getLog(getClass());

	public Cepr01040218Business(EproposalManager eproposalManager, Cepr00000000EditorUtil editorUtil, Object command) {
		super(eproposalManager, editorUtil, command);
		setSekaligusRightPartCdArr(new Integer[] { Cepr01040218Mapper.X3, Cepr01040218Mapper.X6 });
		setBerkalaRightPartCdArr(new Integer[] { Cepr01040218Mapper.X1, Cepr01040218Mapper.X2, Cepr01040218Mapper.X4,
				Cepr01040218Mapper.X5, Cepr01040218Mapper.X7, Cepr01040218Mapper.X8 });
		setInsuredAgeLimit(100);

		String productLabel = "EXCELLINK";

		setInvestmentTitleArr(new String[] { "", productLabel + " Fixed Income Syariah Fund",
				productLabel + " Dynamic Syariah Fund", productLabel + " Aggressive Syariah Fund",
				productLabel + " Secure Dollar Syariah Income", productLabel + " Dynamic Dollar Syariah Fund" });
		setInvestmentTitleAndDescrArr(new String[][] { { "", "", "" }, { "",
				"- " + productLabel + " Fixed Income Syariah Fund : 100% Fixed Income",
				"  Penempatan dana pada Investasi berpendapatan tetap dan instrumen pasar uang yang \n"
						+ "  berdasarkan Syariah dan yang telah mendapatkan sertifikasi syariah dari badan yang berwenang." },
				{ "", "- " + productLabel + " Dynamic Syariah Fund : minimum 50% Fixed Income dan maksimum 50% Ekuitas",
						"  Penempatan dana pada investasi pendapatan tetap, ekuitas serta instrumen pasar uang yang \n"
								+ "  berdasarkan Syariah dan yang telah mendapatkan sertifikasi syariah dari badan yang berwenang." },
				{ "", "- " + productLabel
						+ " Aggressive Syariah Fund : maksimum 100% Fixed Income Rupiah atau maksimum",
						" 50% Fixed  Income US Dollar atau maksimum 100% Ekuitas yang berdasarkan Syariah" },
				{ "", "- " + productLabel + " Secure Dollar Income Syariah Fund : 100% Fixed Income.", null },
				{ "", "- " + productLabel
						+ " Dynamic Dollar Syariah Fund : minimum 50% Fixed Income dan maksimum 50% Ekuitas.",
						null }, });
	...
	public List<Map<String, Object>> getCommonHeaderRowList() throws IOException {
		...
		if (sekaligusRightPartCdList.contains(cepr01030102Form.getRightPartOfBusinessCd())) {
			...
			params = new HashMap<String, Object>();
			if (cepr01030102Form.getPaymentModeCd().toString().equals("3")) {
				params.put("label", "Kontribusi/Premi Pokok Tahunan");
			} else if (cepr01030102Form.getPaymentModeCd().toString().equals("6")) {
				params.put("label", "Kontribusi/Premi Pokok Bulanan");
			}
			if (cepr01030102Form.getPaymentModeCd().toString().equals("2")) {
				params.put("label", "Kontribusi/Premi Pokok Semesteran");
			}
			if (cepr01030102Form.getPaymentModeCd().toString().equals("1")) {
				params.put("label", "Kontribusi/Premi Pokok Triwulanan");
			}
			...
			if (!cepr01030102Form.getTopUpPremium().toString().equals("0")) {
				params = new HashMap<String, Object>();
				if (cepr01030102Form.getPaymentModeCd().toString().equals("3")) {
					params.put("label", "Kontribusi/Premi Top Up Berkala Tahunan");
				} else if (cepr01030102Form.getPaymentModeCd().toString().equals("6")) {
					params.put("label", "Kontribusi/Premi Top Up Berkala Bulanan");
				}
				if (cepr01030102Form.getPaymentModeCd().toString().equals("2")) {
					params.put("label", "Kontribusi/Premi Top Up Berkala Semesteran");
				}
				if (cepr01030102Form.getPaymentModeCd().toString().equals("1")) {
					params.put("label", "Kontribusi/Premi Top Up Berkala Triwulanan");
				}
				...
				params = new HashMap<String, Object>();
				if (cepr01030102Form.getPaymentModeCd().toString().equals("3")) {
					params.put("label", "Total Kontribusi/Premi Tahunan");
				} else if (cepr01030102Form.getPaymentModeCd().toString().equals("6")) {
					params.put("label", "Total Kontribusi/Premi Bulanan");
				}
				if (cepr01030102Form.getPaymentModeCd().toString().equals("2")) {
					params.put("label", "Total Kontribusi/Premi Semesteran");
				}
				if (cepr01030102Form.getPaymentModeCd().toString().equals("1")) {
					params.put("label", "Total Kontribusi/Premi Triwulanan");
				}
				params.put("content", commonUsedBusiness.toMoneyWithCurrencyCd(cepr01030102Form.getPremium()));
				result.add(params);
			}
	...
	public Map<String, Object> getIllustration211Result(int lamaBayar) {
		...
		// if istr_prop.bisnis_id = 120 and (istr_prop.bisnis_no >= 4 and
		// istr_prop.bisnis_no <= 6) then ldec_mfc = 25000
		...
		if (Hardcode.CUR_USD_CD.equals(cepr01030102Form.getCurrencyCd())) {
			ldec_mfc = 2;
			// If Pos('120, 127, 128, 129, 202', string(istr_prop.bisnis_id, '00#')) > 0
			// then
			if (istr_prop.bisnis_id == 120 || istr_prop.bisnis_id == 127 || istr_prop.bisnis_id == 128
					|| istr_prop.bisnis_id == 129 || istr_prop.bisnis_id == 202) {
				if (istr_prop.bisnis_no >= 4 && istr_prop.bisnis_no <= 6)
					ldec_mfc = 2.5;
				if (istr_prop.bisnis_no >= 7 && istr_prop.bisnis_no <= 21)
					ldec_mfc = 2;
			}
			li_bagi = 1;
		} else {
		...
		/*
		 * if( cepr01030102Form.getLeftPartOfBusinessCd() == 141 ) { ldec_mfc = 45000;
		 * if( Hardcode.CUR_USD_CD.equals( cepr01030102Form.getCurrencyCd() ) ) ldec_mfc
		 * = 5; }
		 */
		...
		int a, b;
		/*
		 * if( "01".equals( istr_prop.kurs_id ) ) { a = 1; b = 3; } else { a = 4; b = 5;
		 * }
		 */
		...
		for (int i = 1; i <= istr_prop.ins_per; i++) {
			...
			for (int k = 1; k <= 3; k++) {
				...
				ldec_hasil_invest[1][k] = FormatNumber
						.round(ldec_hasil_invest[1][k] - (new Double(ldec_wdrawbd.toString()) * (1 + ldec_sc)), 2);

				// REQ: MAYDA 07/10/2014
				// Layak Jual, jika: 1.Perhitungan Nill (**) terjadi pd usia TTG pada Ilustrasi
				// Perkembangan Dana /usia Tertanggung pd tahun Polis, usia > 70 thn
				// 2.Utk usia masuk TTG (usia statis) > 60 THN, LAYAK JUAL DIIJINKAN JIKA
				// Ternyata ADA NILAI YG NIL(**) PD ILUSTASI PERKEMBANGAN DANA DIATAS
				// PERIODE/THN POLIS KE 10
				if (istr_prop.bisnis_id == 202 && ((istr_prop.bisnis_no >= 4 && istr_prop.bisnis_no <= 8))) {
					/*
					 * if(j <= 70){ if( ldec_hasil_invest[ 1 ][ k ] <= 0 ) { double
					 * ldec_maks_premi_tahunan = 100000000.0; if( istr_prop.umur_tt>=60 ){ if( i <=
					 * 10 ){ lb_minus[ k ] = true; } }else if( i > 20 && ldec_premi_setahun >=
					 * ldec_maks_premi_tahunan ){ lb_minus[ k ] = false; }else{ lb_minus[ k ] =
					 * true; } } }
					 */
					// REQ: VITO 23/04/2015 : SE No.013/AJS-SK/IV/2015
					// SIMPOL, Layak Jual, jika:
					// 1. Terdapat **/nil bukan pada tahun pertama di asumsi rendah, tapi tidak ada
					// sama sekali **/nil di asumsi Tinggi
					// (terdapat **/nil selain tahun ke-1 di asumsi rendah tapi tidak ada **/nil
					// sampai usia 99 di asumsi Tinggi)
					if (i == 1) {
					...
					if (ldec_hasil_invest[1][k] <= 0) {
						...
						if (i > 20 && ldec_premi_setahun >= ldec_maks_premi_tahunan) {
							lb_minus[k] = false;
						} else {
							lb_minus[k] = true;
						}
				...
				if (i < topupDrawVOListSize) {
					...
					if (topupDrawVO.getYearFlag() != null && topupDrawVO.getYearFlag().equals("true")) {
						if (Hardcode.CUR_USD_CD.equals(istr_prop.kurs_id)) {
							...
							topup = editorUtil
									.convertToString(topupDrawVO.getTopupAmount().divide(new BigDecimal("1000")));
							draw = editorUtil
									.convertToString(topupDrawVO.getDrawAmount().divide(new BigDecimal("1000")));
						}
					}
					if ("0".equals(topup))
						topup = "0.00";
	...
	public List<Map<String, Object>> getCommonBiayaRincianRider(String[][] riderTambahan, int rider_baru)
		...
		if (namaRiders != null && process) {
			...
			params.put("content",
					editorUtil.convertToString("Rp." + riderTambahan[posisiRider][1] + ",- Per Bulan **"));
