...
public class Cepr01040216Business extends Cepr01040000Ilustration134Business
{
    protected final Log logger = LogFactory.getLog( getClass() );

    public Cepr01040216Business( EproposalManager eproposalManager, Cepr00000000EditorUtil editorUtil, Object command )
    {
        super( eproposalManager, editorUtil, command );
        setSekaligusRightPartCdArr( new Integer[]{ } );
        setBerkalaRightPartCdArr( new Integer[]{ Cepr01040216Mapper.X1 } );
    ...
    public List<Map<String, Object>> getCommonHeaderRowList() throws IOException
    {
        ...
        {
            params = new HashMap<String, Object>();
            if( cepr01030102Form.getPaymentModeCd() == 2 ){ // semesteran
...
//        			ldec_hasil_ppokok[k] = (ldec_ppokok + ldec_hasil_ppokok[k]) * (Math.pow((1 + ldec_bunga_avg[k]),( ( double ) 1/12)));
        			...
        			ldec_hasil_ppokok[k] = FormatNumber.round(( ldec_ppokok + ldec_hasil_ppokok[k]) * FormatNumber.round(Math.pow( ( 1 + ldec_bunga_avg[ k ] ), ( ( double ) 1 / 12 )), 15),2);
        		  	if( i > 1 && li_bulan == 1){
        		  		ldec_basic_premium_bd[k] = ( ldec_ppokok ) * FormatNumber.round(Math.pow( ( 1 + ldec_bunga_avg[ k ] ), ( ( double ) 1 / 12 )), 15);
        		  		ldec_topup_bd[k] = (ldec_ptu) * (Math.pow((1 + ldec_bunga_avg[k]),( ( double ) 1/12)));
                	}
        			...
        			if( "01".equals(istr_prop.kurs_id) ){ // perhitungan biaya pokok( foundation fee )
            	    	if(ldec_hasil_ptu[k]>0.0) ldec_ff = (ldec_hasil_ptu[k] * 0.03 / 12);
            	    	if(i <= 8) ldec_ff += (ldec_hasil_ppokok[k] * 0.03 / 12);
        			}else{
        				if(ldec_hasil_ptu[k]>0.0) ldec_ff = (ldec_hasil_ptu[k] * 0.04 / 12);
...
//        	    			double penarikan = ldec_wdraw;
//        	    			if ( istr_prop.pct_premi == 100.0 )
//        	    			{
//        	    				penarikan = penarikan / 2;
//        	    			}
...
// 			  if( i<= 15 ){
//      			ldec_hasil_ppokok[k] += (ldec_hasil_ppokok[k] - ldec_bpp[i]);
// 			  }
