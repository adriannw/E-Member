...
(function(a) {
	a.fn.featureCarousel = function(f) {
		...
		var b = function() {
			...
			if (q.featuresContainer.find(".carousel-feature").first().css(
					"borderLeftWidth") != "medium") {
				q.borderWidth = parseInt(q.featuresContainer.find(
						".carousel-feature").first().css("borderLeftWidth")) * 2
			}
			q.featuresContainer.find(".carousel-feature").each(
					function() {
						a(this).css(
								{
									left : (q.containerWidth / 2)
											- (q.smallFeatureWidth / 2)
											- (q.borderWidth / 2),
			...
			if (f.captionBelow) {
				q.featuresContainer.find(".carousel-caption").css("position",
						"relative")
			}
		...
		var h = function() {
			...
			if (f.trackerSummation) {
				...
				var u = a("<span></span>")
						.addClass("tracker-summation-current").text(
								f.startingFeature);
				var B = a("<span></span>").addClass("tracker-summation-total")
						.text(q.totalFeatureCount);
				var A = a("<span></span>").addClass("tracker-summation-middle")
						.text(" of ");
		...
		var s = function(x, t) {
			if (f.trackerIndividual) {
				var u = q.featuresContainer
						.find(".tracker-individual-container");
			...
			if (f.trackerSummation) {
				var u = q.featuresContainer
						.find(".tracker-summation-container");
		...
		var p = function(u) {
			...
			if (!u && f.autoPlay != 0) {
				var t = (Math.abs(f.autoPlay) < f.carouselSpeed) ? f.carouselSpeed
						: Math.abs(f.autoPlay);
		...
		var o = function(y, C) {
			...
			if (x == 1) {
				...
				v = f.topPadding;
				u = y.css("z-index");
				z = (q.containerWidth / 2) - (q.largeFeatureWidth / 2)
						- (q.borderWidth / 2);
				A = 1
			} else {
				...
				if (x == q.totalFeatureCount) {
					...
					if (x == 2) {
						z = q.containerWidth - q.smallFeatureWidth
								- f.sidePadding - q.borderWidth
					} else {
						z = (q.containerWidth / 2) - (q.smallFeatureWidth / 2)
								- (q.borderWidth / 2);
						A = 0
					}
				}
			...
			y.animate(
					...
					function() {
						...
						if (f.trackerIndividual || f.trackerSummation) {
							if (x == 1) {
								var E = q.featuresContainer.find(
										".carousel-feature").index(y) + 1;
								var F;
								if (C == false) {
									F = i(E)
								} else {
		...
		a(".tracker-individual-blip", q.containerIDTag).live(
				"click",
				function() {
					var t = a(this).attr("id").substring(8);
					var v = q.featuresContainer.find(".carousel-feature").eq(
							t - 1).data("position");
